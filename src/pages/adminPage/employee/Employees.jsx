...
const Employees = () => {
  ...
  const { getEmployees, resetEmployeePassword, deleteEmployee } = useAuth();
  ...
  const fetchEmployee = async (isInitial = false) => {
    const data = {
      page: currentPage,
      limit: itemsPerPage,
    ...
    },
    {
      ...
      cell: row => (
        <Dropdown align="end">
          ...
          <Dropdown.Menu>
            <Dropdown.Item onClick={() => navigate(`/admin/employees/edit/${row.encrypted_id}`)}>
              <Edit size={14} className="me-2" /> Edit
            </Dropdown.Item>
            <Dropdown.Item onClick={() => navigate(`/admin/employees/${row.encrypted_id}`)}>
  ...
  const handleDeleteConfirm = async () => {
    try {
      const res = await deleteEmployee(selectedVendorId); // You should implement this in useAuth
      if (res.success) {
  ...
  return (
    <MainLayout title="AKC Employees">
      {pageLoading ? (
        <PageLoader />
      ) : (
        <>
          <Container fluid className="p-3">
            <Row className="g-4 mb-4">
              <Col xs={12} md={6} lg={3}>
                <StatsCard
                  ...
                  value={employees?.employeeStatistics?.total}
                />
              </Col>
              <Col xs={12} md={6} lg={3}>
                <StatsCard
                  title="ACTIVE EMPLOYEES"
                  value={employees?.employeeStatistics?.active}
                />
              </Col>
              <Col xs={12} md={6} lg={3}>
                <StatsCard
                  title="INACTIVE EMPLOYEES"
                  value={employees?.employeeStatistics?.inactive}
                />
              </Col>
              <Col xs={12} md={6} lg={3}>
                <StatsCard
                  title="LAST 7 DAYS EMPLOYEES"
                  value={employees?.employeeStatistics?.last7Days}
                />
              </Col>
            </Row>

            <NetwordCards data={employees.network} />
            ...
            <Row className="align-items-center justify-content-between mt-3 px-4">
              ...
              <Col md={6} className="text-end">
                <Button variant="primary" onClick={() => navigate('/admin/employees/add')}>
                  <Plus size={14} className="me-1" /> Add Employee
                </Button>
              </Col>
            </Row>
            ...
            <Card className="p-3 border-0 shadow rounded-4 mt-3">
              <Card.Header className="bg-white border-0 mb-2">
                <FilterBar showVendor={true} showEmployee={true} onFilterChange={handleFilterChange} />
              </Card.Header>
             ...
             <div style={{ overflowX: 'auto' }}>
              <DataTable
                ...
                progressComponent={<CustomTransactionLoader text='Loading Employee' />}
                pagination
                ...
                pointerOnHover
                responsive
                noDataComponent="No employees found matching your criteria."
              // dense
              />
              </div>
